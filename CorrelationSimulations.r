...
sampler = function(n.samps, n = 1000, seed = 1453){
  set.seed(seed)
  ints = matrix(ncol = 10)
  falneg = matrix(ncol=3)
  results_list <- mclapply(1:n.samps, function(x){ 
    X = matrixcreator(n)
    y = ycreator(X)
    mod = lm(y ~ X)
    j = confint(mod)
    in_interval = c(j[2,1]<0.5 & 0.5<j[2,2], j[3,1]<0.2 & 0.2<j[3,2], j[4,1] < -0.8 & -0.8 < j[4,2], apply(j[5:11,], 1, function(x) prod(x) < 0))
    falneg = apply(j[2:4,], 1, function(x) prod(x) < 0)
    return(list(in_interval, falneg))
  }, mc.cores = 4)
  for (l in results_list){
    ints = rbind(ints, l[[1]])
    falneg = rbind(falneg, l[[2]])
  }
  
  confint_perc = apply(ints, 2, function(x) mean(x, na.rm = T))
  false_positives = 1- confint_perc[4:10]
  false_negatives = apply(falneg, 2, function(x) mean(x, na.rm = T))
  return(list('confint_perc' = confint_perc, 'false_positives' = false_positives, 'false_negatives' = false_negatives))
}

sampler(1000, 2000, 400)
...
varying_samps <- function(runs, n=500, seed = 500){
  ...
  for (i in 1:length(runs)){
    iteration = sampler(runs[i], n, seed= 12*i)
...
varying_rho <- function(rho_sequence, n.samps=500, n =1000, seed = 100){
  set.seed(seed)
  CI = matrix(ncol = 10, nrow = length(rho_sequence))
  FP = matrix(ncol = 7, nrow = length(rho_sequence))
  FN = matrix(ncol = 3, nrow = length(rho_sequence))
  for (i in 1:length(rho_sequence)){
    iteration = correlationcreator(rho_sequence[i], n.samps, n, seed = 500*i)
    CI[i, ] = iteration$confint_perc
    FP[i, ] = iteration$false_positives
    FN[i, ] = iteration$false_negatives
  }
...
for (i in 2:3) points(runs, CI_plot$CI[,i], col = col[i], type= 'l', lty = i, lwd = 1.5)
abline(h=0.950, lty=2)
legend('topright', legend = c('x1', 'x2', 'x3'), col = col[1:3], lty = 1:3 , cex = 0.8, bty = 'n')
dev.off()

#warning, even parallelized, this rho_plot takes quite some time to run
rho_sequence = seq(0, 0.95, 0.05)
rho_plot = varying_rho(rho_sequence, 7000, 3000)
